...
export class CirRolesDemandCardComponent implements OnInit {
  ...
  getJobFutureRoleList() {
    ...
    this.cardDetails = {};
    this.cirSericeService.getJobRoleList(payload, this.id).subscribe((response) => {
      this.jobRoleList = [];
      if (response?.status == true) {
        ...
        this.jobRoleList?.map((element: any) => {
          this.manchesterDetails.push(
            {
              id: element?._id,
              field: element?.title,
  ...
  submit() {
    ...
    const payload = {
      ...
      location: this.cardDetails?.location, // Fill in the location if needed
      roles: selectedRoles
    };
    ...
    this.cirSericeService.getClientRoles().subscribe((response) => {
      if (response?.status) {
        if (response?.data?.length > 0) {
          const tempData: any[] = response?.data?.filter((item: any) => item.name == this.cardDetails?.name);
